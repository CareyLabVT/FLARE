write_forecast_netcdf <- function(x,
  ...
  def_list <- list()
  def_list[[1]] <- ncvar_def("temp","degC",list(timedim,depthdim, ensdim),fillvalue,'temperature',prec="single")
  def_list[[2]] <- ncvar_def("qt_restart","-",list(statedim,statedim),fillvalue,'restart covariance matrix',prec="float")
  def_list[[3]]  <- ncvar_def("x_restart","-",list(ensdim,stateagudim),fillvalue,'matrix for restarting EnKF',prec="float")
  def_list[[4]] <- ncvar_def("x_prior","-",list(timedim,ensdim,stateagudim),fillvalue,'Predicted states prior to Kalman correction',prec="float")
  def_list[[5]] <- ncvar_def("obs","various",list(timedim,depthdim,obsdim),fillvalue,'observations',prec="single")
  def_list[[6]] <- ncvar_def("data_assimilation","-",list(timedim),missval = -99,longname = '0 = historical; 1 = forecasted',prec="integer")
  def_list[[7]] <- ncvar_def("surface_height_restart","meter",list(ensdim),missval = -99,longname =  'Surface Height',prec="single")
  def_list[[8]] <- ncvar_def("snow_ice_restart","meter",list(ensdim, snow_ice_dim),missval = -99,longname = 'Snow (1), White Ice (2), Blue Ice (3)',prec="single")
  def_list[[9]] <- ncvar_def("ice_thickness","meter", list(timedim,ensdim),missval = -99,longname = 'Ice Thickness',prec="single")
  def_list[[10]] <- ncvar_def("lake_depth","meter",list(timedim,ensdim),missval = -99,longname = 'Depth of lake',prec="single")
  def_list[[11]] <- ncvar_def("avg_surf_temp_restart","degC",list(ensdim),missval = -99,longname ='Running Average of Surface Temperature',prec="single")
  def_list[[12]] <- ncvar_def("running_residuals","various",list(qt_update_days_dim,statedim),fillvalue,longname = "running residual for updating qt",prec="single")
  def_list[[13]] <- ncvar_def("mixing_restart","various",list(ensdim,mixing_restart_vars_dim),fillvalue,longname = "variables required to restart mixing",prec="single")
  def_list[[14]] <- ncvar_def("depths_restart","various",list(ensdim,depth_restart_vars_dim),fillvalue,longname = "depths simulated by glm that are required to restart ",prec="single")
  ...
  if(include_wq){
    for(s in 2:length(state_names)){
      def_list[[index+npars+s-1]]<- ncvar_def(state_names[s],"mmol m-3",list(timedim,depthdim, ensdim),fillvalue,state_names[s],prec="single")
    }
  }
  
  if(length(diagnostics_names) > 0){
    for(s in 1:length(diagnostics_names)){
      def_list[[index+npars+length(state_names)-1 + s]]<- ncvar_def(diagnostics_names[s],"-",list(timedim,depthdim, ensdim),fillvalue,diagnostics_names[s],prec="single")
    }
  }
  ...
  ncvar_put(ncout,def_list[[5]] ,as.array(obs))
  ncvar_put(ncout,def_list[[6]] ,as.array(data_assimilation_flag))
  ncvar_put(ncout,def_list[[7]] ,surface_height_restart)
  ncvar_put(ncout,def_list[[8]] ,snow_ice_restart)
  ncvar_put(ncout,def_list[[9]] ,ice_thickness)
  ncvar_put(ncout,def_list[[10]] ,surface_height)
  ncvar_put(ncout,def_list[[11]] ,avg_surf_temp_restart)
  ncvar_put(ncout,def_list[[12]] ,running_residuals)
  ncvar_put(ncout,def_list[[13]] ,mixing_restart)
  ncvar_put(ncout,def_list[[14]] ,glm_depths_restart)
  ...
  if(length(diagnostics_names) > 0){
    for(s in 1:length(diagnostics_names)){
      ncvar_put(ncout, def_list[[index+npars+length(state_names) - 1 + s]],diagnostics_efi[, , ,s])
    }
  }
  ...
  ncatt_put(ncout,0,"forecast_issue_time",as.character(time_of_forecast), prec =  "text") 
