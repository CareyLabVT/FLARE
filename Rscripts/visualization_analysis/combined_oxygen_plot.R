combined_oxygen_plot <- function(with_oxy, without_oxy, forecast_location, push_to_git){
  ...
  for(i in 1:dim(oxy)[1]){
    oxy_sss_9m_mean[i] <- mean(oxy[i, , 10])
    oxy_sss_9m_low95[i] <- quantile(oxy[i, , 10], 0.025)
    oxy_sss_9m_up95[i] <- quantile(oxy[i, , 10], 0.975)
  }
  ...
  for(i in 1:dim(oxy)[1]){
    oxy_9m_mean[i] <- mean(oxy[i, , 10])
    oxy_9m_low95[i] <- quantile(oxy[i, , 10], 0.025)
    oxy_9m_up95[i] <- quantile(oxy[i, , 10], 0.975)
  }
