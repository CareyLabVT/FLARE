                                       ...
                                       include_wq)
{
  ...
  met_drivers <- read_csv(met_drivers_file, col_types = cols())
  
  
  curr_all_days <- met_drivers %>% 
    group_by(NOAA.member, dscale.member) %>% 
    mutate(ensemble = cur_group_id() - 1) %>% 
    ungroup() %>% 
  ...
  for(i in 1:(n_distinct(tmp$ensemble) - 1)){
    tmp2 <- tmp %>% 
      filter(ensemble == i | ensemble == 0) %>% 
      mutate(SALT = 0.0) %>% 
  ...
  return(list(inflow_file_names = tmp2,
              spillway_file_names = tmp2_outflow,
              wetland_file_names = NA))
}
